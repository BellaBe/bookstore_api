...
const server = restify.createServer(lib.config.server);
...
server.use(function(req, res, next){
    //We move forward if we are dealing with the swagger-ui or a valid key
...
//we remove the {format} part of the paths, to
swagger.configure("https://localhost:9000", "0.1")
...
server.listen(config.server.port, function(){
    ...
    lib.db.connect(function(err){
        if(err) console.log('Error trying to connect do database: '.red, err.red)
        else console.log("Database service succesfuy started".red)
    })
})
