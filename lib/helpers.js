...
function setupRoutes(server, swagger, lib){
    for(controller in lib.controllers){
        ...
        ont.setUpActions(server, swagger)
    }
}

function validateKey(hmacdata, key, lib){
    //This is for testing the swagger-ui, shold be removed after development to avoid possible security problem
...
function make HAL (data, links, embed){
    var obj = halson(data)
